...
var sync_default = async (req) => {
  ...
  const a = body && body.a, k = body && body.k;
  if (typeof k !== "string" || !/^[a-f0-9]{64}$/.test(k)) return json({ error: "bad key" }, 400);
  const store = getStore({ name: "academy-sync", consistency: "strong" });
  try {
    if (a === "get") {
      const r = await store.getWithMetadata(k, { type: "json" });
      return json(r ? { state: r.data, etag: r.etag } : { state: null, etag: null });
    }
    if (a === "put") {
      const st = body.state;
      if (!st || typeof st !== "object" || typeof st.led !== "object") return json({ error: "bad state" }, 400);
      const payload = JSON.stringify(st);
      const res = body.etag ? await store.set(k, payload, { onlyIfMatch: body.etag }) : await store.set(k, payload, { onlyIfNew: true });
      if (!res.modified) return json({ conflict: true }, 409);
